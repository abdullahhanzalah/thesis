from transformers import AutoImageProcessor, AutoModel
from PIL import Image
import requests
import torch
from sam2.sam2.build_sam import build_sam2_video_predictor

def load_dino_model():
    processor = AutoImageProcessor.from_pretrained('facebook/dinov2-base')
    model = AutoModel.from_pretrained('facebook/dinov2-base')
    return processor, model

def get_dino_embeddings(image, model, processor):
    inputs = processor(images=image, return_tensors="pt")
    print(inputs.pixel_values.shape)  # [1, 3, 224, 224]
    batch_size, rgb, img_height, img_width = inputs.pixel_values.shape
    num_patches_height, num_patches_width = img_height // model.config.patch_size, img_width // model.config.patch_size
    num_patches_flat = num_patches_height * num_patches_width

    outputs = model(**inputs)
    last_hidden_states = outputs[0]
    print(last_hidden_states.shape)  # [1, 1 + 256, 768]
    print("here")
    assert last_hidden_states.shape == (batch_size, 1 + num_patches_flat, model.config.hidden_size)

    cls_token = last_hidden_states[:, 0, :]
    patch_features = last_hidden_states[:, 1:, :].unflatten(1, (num_patches_height, num_patches_width))
    print("here")
    return cls_token, patch_features


def load_image():
    url = 'http://images.cocodataset.org/val2017/000000039769.jpg'
    image = Image.open(requests.get(url, stream=True).raw)
    print(image.height, image.width)  # [480, 640]
    return image

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
processor, model = load_dino_model()
image = load_image()
cls_token, patch_features = get_dino_embeddings(image, model, processor)
print(cls_token.shape)  # [1, 768]

# predictor = build_sam2_video_predictor("sam2_hiera_t", "sam2/checkpoints/sam2_hiera_tiny.pt", device=device)