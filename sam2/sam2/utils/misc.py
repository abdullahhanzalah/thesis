...
#     print(f"Input shape: {np_img.shape}, dtype: {np_img.dtype}")
#     if np_img.ndim == 2:
...
def _load_np_as_tensor(np_img, image_size):
    """Fixed version of _load_np_as_tensor"""
    print(f"Input shape: {np_img.shape}, dtype: {np_img.dtype}")
    
    # Handle different data types and normalize to [0, 1] range
    if np_img.dtype != np.uint8:
        # For float images, ensure they're in [0, 1] range
        if np_img.max() > 1.0:
            # If values are > 1, assume they need normalization
            img_min, img_max = np_img.min(), np_img.max()
            if img_max > img_min:
    ...
    # Handle different input formats and convert to 2D first
    if np_img.ndim == 2:
        # Already 2D grayscale (H, W) - this is what we want
        pass
    elif np_img.ndim == 3:
        if np_img.shape[0] == 1:
            ...
            # Shape is (3, H, W) - convert to grayscale
            np_img = np.mean(np_img, axis=0).astype(np.uint8)
        elif np_img.shape[2] == 3:
            # Shape is (H, W, 3) - convert to grayscale
            np_img = np.mean(np_img, axis=2).astype(np.uint8)
        else:
            # Other 3D shapes, take the first slice or channel
            if np_img.shape[0] < np_img.shape[2]:
                np_img = np_img[0]  # Take first channel
            else:
                np_img = np_img[:, :, 0]  # Take first channel
    else:
        raise ValueError(f"Cannot handle image with {np_img.ndim} dimensions")
    
    # Ensure we have 2D grayscale image at this point
    if np_img.ndim != 2:
        raise ValueError(f"After processing, image should be 2D, got shape: {np_img.shape}")
    
    print(f"After processing: {np_img.shape}, dtype: {np_img.dtype}")
    
    # Convert to RGB format (H, W, 3) for PIL
    np_img_rgb = np.stack([np_img, np_img, np_img], axis=-1)
    
    print(f"RGB shape for PIL: {np_img_rgb.shape}")
    ...
    # Convert to [0, 1] range for tensor
    if img_np.dtype == np.uint8:
        img_np = img_np / 255.0
    else:
        raise RuntimeError(f"Unexpected image dtype after PIL conversion: {img_np.dtype}")
